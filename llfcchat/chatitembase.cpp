    ...
    , m_role(role)
{
    ...
    if(m_role == ChatRole::Self)
    {
        ...
        pGLayout->addWidget(m_pNameLabel, 0, 1, 1, 1); // 第0行第1列占1行占1列
        pGLayout->addWidget(m_pIconLabel, 0, 2, 2,1, Qt::AlignTop); // AlignTop顶部对齐
        pGLayout->addItem(pSpacer, 1, 0, 1, 1);
        pGLayout->addWidget(m_pBubble, 1, 1, 1, 1);

        pGLayout->setColumnStretch(0, 2); // 第0列占比例是2
        pGLayout->setColumnStretch(1, 3); // 第1列占比例是3
    }else{
